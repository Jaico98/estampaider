document.addEventListener("DOMContentLoaded", () => {
    ...
    function animarContador(id, valorFinal, esDinero = false) {
      ...
      let valor = 0;
      const incremento = Math.max(1, Math.ceil((Number(valorFinal) || 0) / 30));
    ...
    function obtenerTipoCliente(telefono) {
      ...
      const pedidosCliente = pedidosGlobal.filter(
        (p) => normalizarTelefono(p.telefono) === normalizarTelefono(telefono) && p.estado === "ENTREGADO"
      );
    ...
    function actualizarDashboard(pedidos) {
      ...
      const pendientes = pedidos.filter((p) => p.estado === "PENDIENTE" || p.estado === "RECIBIDO").length;
      ...
      if (window.Chart && document.getElementById("graficaEstados")) {
        chartEstados = new Chart(document.getElementById("graficaEstados"), {
          ...
          data: {
            ...
            datasets: [{ data: Object.values(estados), backgroundColor: ["#facc15", "#3b82f6", "#22c55e"] }],
          },
        ...
        chartVentas = new Chart(document.getElementById("graficaVentas"), {
          ...
          options: {
            ...
            onClick: (_, elements) => {
              ...
              paginaActual = 1;
              filtros.forEach((b) => b.classList.toggle("activo", b.dataset.estado === "ENTREGADO"));
              renderPedidos();
            },
    ...
    function calcularTotalPagado(pedidos) {
      ...
      const el = document.getElementById("statPagado");
    ...
    function crearPedidoCard(pedido) {
      ...
      fecha.textContent = `Fecha: ${pedido.fecha ? new Date(pedido.fecha).toLocaleString("es-CO") : "—"}`;
      ...
      if (pedido.detalles?.length) {
        pedido.detalles.forEach((d) => {
          ...
          li.textContent =
            `${textoSeguro(d.producto)} x ${Number(d.cantidad) || 0} — $${(
      ...
      pago.textContent = `Estado de pago: ${pedido.estadoPago === "PAGADO" ? "PAGADO" : "PENDIENTE"}`;
      ...
      const btnEntregado = crearBoton("Entregado", "estado", () => actualizarEstado(pedido.id, "ENTREGADO"));
      ...
      if (pedido.estadoPago !== "PAGADO") {
        acciones.appendChild(
          crearBoton("Marcar pagado", "pagar", () => marcarPagado(pedido.id))
        );
      }
  
      acciones.appendChild(
        crearBoton("Eliminar", "eliminar", () => eliminarPedido(pedido.id))
      );
    ...
    function cargarPedidos() {
      fetch(`${API_BASE}/api/pedidos`, { headers: getAuthHeaders() })
        ...
        .catch(() => {
          contenedor.textContent = "Error al cargar pedidos";
